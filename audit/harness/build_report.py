...
<div class="callout"><strong>What has changed on the published list, and what has not.</strong> As of
<code>v0.2.0</code>, deployed 2026-08-25, the corrections below are <strong>live</strong> at
clustermap.vibingco.de — and none of them changes the clustering. It is still SybilKit 0.1.1, still 263
groups, still 11,573 wallets linked. What changed is what the page claims about them.
<strong>Done:</strong> nonce and first funder enriched for every contributor; the v2 rules measured;
a wallet held by fewer than two evidence families now renders as <em>review</em> rather than inheriting
its group's tier (1,107 wallets move, and the wallet quoted at the top of this report drops from
critical 0.97 to review); tier wording is group-scoped, so a wallet page states membership rather than a
verdict; no evidence link is drawn stronger than the wallets it joins; the list export carries its own
provenance and known defects, and every row carries the evidence families incident on that wallet; and a
dispute route is published, stating the measured error rate up front.
<strong>Not done, deliberately:</strong> the v2 rule set is <em>not</em> adopted — it will ship as a
selectable version so it can be inspected before it changes anyone's status, and the default flips only
in its own release. And the dispute route has no public dated overrides file yet, so an accepted dispute
currently changes nothing visible; that needs the version model. Until then a contest references a
release tag, and every wallet's status is recomputable from published data by anyone.
The harness, diagnostics, null model and complete enrichment behind every number here are at
<code>github.com/banse/clustermap</code> under <code>audit/</code>, and reproduce from a clone.</div>
...
<li><strong>Everything here reproduces from a public clone</strong> — <code>github.com/banse/clustermap</code>, directory <code>audit/</code> — with no private input, no API key and no network. Reproduction covers cluster <em>membership</em>, not totals: the sorted membership of all v2 clusters hashes to <code>bd986908e33bf6c1c4cda481dae0009f</code> and the flagged set to <code>71e561a2d104bea9f0e36e742ec54ddc</code>, identically across machines. So an individual verdict can be checked and disputed rather than an aggregate taken on trust. The population itself is being made rebuildable directly from the contract's event log, which removes the last file that is ours rather than the chain's; the one input that can never come from the contract is each wallet's first funder, since that is a fact about the wallet and not about the game.</li>
